...
function WhatsNewCard({ thing, onOpen }: { thing: RadarThing; onOpen: (t: RadarThing) => void }) {
  ...
  return (
    <motion.button {...tap} whileTap={{ scale: 0.97 }} transition={{ type: "spring", stiffness: 440, damping: 28 }} style={{ display: "flex", flexDirection: "column", textAlign: "left", background: SURFACE, border: `1px solid ${HAIRLINE}`, borderRadius: "16px", padding: "10px", cursor: "pointer", color: "inherit", boxShadow: INNER_HIGHLIGHT, minWidth: 0, overflow: "hidden", width: "100%" }}>
      ...
      {/* Name and category share one line — "box · Dev tools" — instead of stacking */}
      <div style={{ display: "flex", alignItems: "center", gap: "6px", marginTop: "4px", minWidth: 0, width: "100%" }}>
        <span style={{ flexShrink: 0, maxWidth: "100%", fontSize: "14.5px", fontWeight: 600, color: TEXT.primary, letterSpacing: "-0.01em", lineHeight: 1.2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{thing.name}</span>
        {meta && (
          <>
            <span aria-hidden style={{ flexShrink: 0, width: "4px", height: "4px", borderRadius: "50%", background: accent.ring }} />
            <span style={{ minWidth: 0, fontSize: "11.5px", fontWeight: 500, color: TEXT.muted, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{meta}</span>
          </>
        )}
      </div>
...
function Section({ emoji, eyebrow, sub, variant, things, onOpen, onSeeAll }: SectionData & { onOpen: (t: RadarThing) => void; onSeeAll?: () => void }) {
  ...
  return (
    <section style={{ marginBottom: "30px" }}>
      ...
      {variant === "rail" ? (
        isDesktop ? (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: "12px", padding: "0 24px" }}>
            {things.map((t) => <RailCard key={t.id} thing={t} wide={false} fill onOpen={onOpen} />)}
          </div>
        ) : (
          <div className="scrollbar-none radar-rail" style={{ display: "flex", gap: "12px", padding: "0 24px", overflowX: "auto", scrollSnapType: "x proximity" }}>
